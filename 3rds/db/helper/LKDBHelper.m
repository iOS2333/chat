...
//    return [self initWithDBName:@"LKDB"];
        return [self initWithDBName:SG_DBNAME];
}
...
- (NSMutableArray *)executeResult:(FMResultSet *)set Class:(Class)modelClass tableName:(NSString *)tableName
{
    ...
    while ([set next]) {
        ...
        //ly add
        NSArray *notBindings = [modelClass sqlNotToBindingPropertyValue];
        
        for (int i = 0; i < columnCount; i++) {
            NSString *sqlName = [set columnNameForIndex:i];
            NSLog(@"sqlName:%@",sqlName);
            
            //ly add
            if ([notBindings containsObject:sqlName]) {
                continue;
            }
...
//                    if ([columnClass isSubclassOfClass:[UIImage class]]) {
//                        NSString *imagePropertyPathName = [NSString stringWithFormat:@"%@Path",property.propertyName];
//                        LKDBProperty *imagePropertyPath = [infos objectWithPropertyName:imagePropertyPathName];
//                        if (imagePropertyPath) {
//                            [bindingModel modelSetValue:imagePropertyPath value:sqlValue];
//                        }
//                    }
...
-(BOOL) delete_by_where:(NSString *) sql model:(NSObject *) model{
    ...
    NSString *deleteSQL = [NSString stringWithFormat:@"delete from %@ where  %@", db_tableName,sql];
...
//    }
    ...
    BOOL execute = [self executeSQL:deleteSQL arguments:nil];
