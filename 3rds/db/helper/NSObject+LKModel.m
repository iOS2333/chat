...
+(BOOL) deleteDBImageWithName:(NSString *) filename{
...
+ (void)getSelfPropertys:(NSMutableArray *)pronames protypes:(NSMutableArray *)protypes
{
    ...
    //ly add
    NSDictionary *ddic = [self getColumnTypeByYourself];
    if (ddic != nil && [[ddic allKeys] count]>0 ) {
        
        NSArray *keys = [ddic allKeys];
        for (int i = 0; i<[keys count]; i++) {
            NSString *key = [keys objectAtIndex:i];
            NSInteger index = [pronames indexOfObject:key];
            NSString *newval = [ddic objectForKey:key];
            [protypes replaceObjectAtIndex:index withObject:newval];
        }
